...
Proof.
  induction n; induction m; induction p; simpl; auto with arith;
  intros H; apply lt_n_0 in H; solve [ case H ].
Qed.
...
Proof.
  unfold lt. intros. change (S n + p <= m + q).
  cut (n + S p <= m + q).
  - clear; intro H'.
    generalize dependent (m + q).
    clear; intros m H.
    revert n p m H.
